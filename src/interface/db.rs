...
pub trait Getter {
    ...
    async fn get(&self, bucket: &str, key: &[u8]) -> anyhow::Result<Option<Bytes>>;
    ...
    async fn get_index_chunk(&self, bucket: &str, key: &[u8], timestamp: u64) -> anyhow::Result<Option<Bytes>>;

    /// Has indicates whether a key exists in the database.
    async fn has(&self, bucket: &str, key: &[u8]) -> anyhow::Result<bool>;
    ...
    async fn walk(&self, bucket: &str, startkey: &[u8], fixedbits: usize) -> Self::WalkStream<'_>;

    /// MultiWalk is similar to multiple Walk calls folded into one.
    async fn multi_walk(&self, bucket: &str, startkey: &[&[u8]], fixedbits: &[usize]) -> Self::MultiWalkStream<'_>;
}
...
pub trait Database: Getter + Putter + Deleter {
    ...
    async fn last(&self, bucket: &str) -> anyhow::Result<(Bytes, Bytes)>;
    ...
    async fn append(&self, bucket: &str, key: &[u8], value: &[u8]) -> anyhow::Result<()>;

    async fn append_dup(&self, bucket: &str, key: &[u8], value: &[u8]) -> anyhow::Result<()>;
...
pub trait DbWithPendingMutations: Database {
    ...
    async fn reserve(&self, bucket: &str, key: &[u8], i: usize) -> anyhow::Result<Bytes>;
}