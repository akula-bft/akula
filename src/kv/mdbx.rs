...
fn map_res_inner<Key, Value, E>(
    v: Result<Option<(TableObjectWrapper<Key>, TableObjectWrapper<Value>)>, E>,
) -> Result<Option<(Key, Value)>, E>
where
    Key: TableDecode,
    Value: TableDecode,
{
    v.map(|opt| opt.map(|(wk, wv)| (wk.0, wv.0)))
}
    ...
    T: Table,
{
    ...
    async fn delete(&mut self, key: T::Key, value: T::Value) -> anyhow::Result<()> {
        if self.table_info.dup_sort {
            if self
                .inner
                .get_both::<Bytes<'txn>>(key.encode().as_ref(), value.encode().as_ref())?
                .is_some()
            {
